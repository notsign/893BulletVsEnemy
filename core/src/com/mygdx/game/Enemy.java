...
public class Enemy extends Actor {
	...
	Animation idle, run;
	float enemywidth = taIdle.findRegion("idle (1)").getRegionWidth();
	float enemyheight = taIdle.findRegion("idle (1)").getRegionHeight();
	float elapsedTime = 0;
	int health;
	...
	private void createFootSensor() {
		...
		shape.setAsBox(enemywidth / 4, 0.2f, new Vector2(enemyMainBody.getLocalCenter().x, enemyMainBody.getLocalCenter().y - (enemyheight / 4) - 0.1f), 0);
